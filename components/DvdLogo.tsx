...
const DvdLogo: React.FC<DvdLogoProps> = ({ containerRef, fps, effectsConfig, config, onPlaySfx }) => {
  ...
  // OPTIMIZATION: Cache container dimensions to avoid getBoundingClientRect() (layout thrashing) in the loop
  const containerDims = useRef({ w: 0, h: 0 });

  // Determine aspect ratio based on logo type
  const aspectRatio = useMemo(() => {
      if (config.logoType === 'neon_waves') return (740.376 / 805);
      if (config.logoType === 'custom') return 1; // Default to square if custom, will fit-contain
      return 0.5; // DVD default 2:1
  }, [config.logoType]);

  // --- RESIZE OBSERVER ---
  // Efficiently track container size changes without forcing reflows every frame
  useEffect(() => {
      const updateDims = () => {
          if (containerRef.current) {
              // Use clientWidth/Height which are faster than getBoundingClientRect for this purpose
              // and don't include borders/scrollbars usually, which is good for containment
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      const contW = containerDims.current.w;
      const contH = containerDims.current.h;
      ...
      setPosition((prev) => {
        ...
        // OPTIMIZATION: Calculate current dimensions mathematically instead of DOM read
        // This avoids layout thrashing.
        const currentW = config.size;
        const currentH = config.size * aspectRatio;
  ...
  }, [fps, effectsConfig, config.speed, config.size, config.enableSfx, onPlaySfx, aspectRatio]); 

  // RENDER CONTENT
  const renderContent = () => {
      if (config.logoType === 'custom' && config.customLogoUrl) {
          return (
              <img 
                  src={config.customLogoUrl} 
                  alt="DVD Logo" 
                  className="w-full h-full object-contain drop-shadow-[0_0_10px_currentColor]"
                  style={{ color: color }} // Just in case, though img ignores it unless SVG
              />
          );
      }

      if (config.logoType === 'neon_waves') {
          return (
...
export default React.memo(DvdLogo);