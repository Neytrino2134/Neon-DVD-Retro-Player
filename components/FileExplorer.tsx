...
const FileExplorer: React.FC<FileExplorerProps> = ({ isOpen, onClose, mode, onSelect }) => {
  const [currentPath, setCurrentPath] = useState<CatalogItem[]>(CATALOG_ROOT);
  const [selectedItems, setSelectedItems] = useState<Set<CatalogItem>>(new Set());

  // Reset when opening
  useEffect(() => {
    if (isOpen) {
      setCurrentPath(CATALOG_ROOT);
  ...
  const toggleSelection = (item: CatalogItem) => {
    ...
    // Music mode: Multi select or Album select
    else {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-3xl h-[80vh] bg-gray-900 border-2 border-neon-green shadow-[0_0_30px_rgba(0,255,0,0.2)] flex flex-col rounded overflow-hidden font-mono">
        ...
        {/* Path Bar */}
        <div className="bg-black border-b border-gray-800 p-2 text-neon-green/80 flex items-center gap-2">
        ...
        <div className="bg-gray-900 border-t border-neon-green p-3 flex justify-between items-center">
           ...
           <div className="flex gap-2">
             ...
             <button
               ...
               `}
             >
               {mode === 'music' ? 'LOAD TRACKS' : 'APPLY BACKGROUND'}
             </button>
           </div>
        </div>
