...
import { VisualizerConfig, DvdConfig, EffectsConfig, MarqueeConfig, PatternConfig, BackgroundMedia, BackgroundPlaylist, AppPreset, CursorStyle, WatermarkConfig, ThemeType, ControlStyle, BgTransitionType, BgAnimationType, BgHotspot, EqualizerConfig, FitMode, ScreenAlignment, YouTubeAuthConfig, TerrainConfig, RoadConfig } from '../types';
...
import { 
  ...
  DEFAULT_ROAD_CONFIG, // NEW
  DEFAULT_GLOBAL_WAVEFORM_CONFIG, 
  DEFAULT_DVD_CONFIG, 
...
const STORAGE_KEYS = {
  ...
  SYNC_BG_WITH_TRACK: 'neon_sync_bg_with_track'
};
...
const SECTION_MODULES: Record<string, string[]> = {
  ...
  waves: ['master_wave', 'wave', 'reactor', 'sine', 'terrain', 'road'], // Added road
...
export const useAppConfig = () => {
  ...
  const [showRoad, setShowRoad] = useState(() => getInitial(STORAGE_KEYS.SHOW_ROAD, false)); // NEW
  ...
  const [roadConfig, setRoadConfig] = useState<RoadConfig>(() => getInitial(STORAGE_KEYS.ROAD, DEFAULT_ROAD_CONFIG)); // NEW
  ...
  // ... (rest of background logic unchanged)
  ...
  const [syncBgWithTrack, setSyncBgWithTrack] = useState<boolean>(() => getInitial(STORAGE_KEYS.SYNC_BG_WITH_TRACK, false)); 
  const [timerResetToken, setTimerResetToken] = useState(0);
  const [activePresetId, setActivePresetId] = useState<string | null>(() => localStorage.getItem(STORAGE_KEYS.ACTIVE_PRESET) || 'default_system');
  ...
  const [settingsOpenSections, setSettingsOpenSections] = useState<Record<string, boolean>>({
      sys: true, bg: true, sfx: true, waves: true, mod: true, game: true, post: true
  });
  ...
  const toggleSettingsExpand = (id: string, isAdditive: boolean, forceOpen?: boolean) => {
        setSettingsExpandedState(prev => {
            ...
            if (isAdditive && forceOpen === undefined) {
                ...
                groupIds.forEach(siblingId => { if (siblingId !== id) newState[siblingId] = false; });
  ...
  const toggleSettingsSection = (sectionId: string, isAdditive: boolean = false) => {
      setSettingsOpenSections(prev => {
          ...
          if (!isCurrentlyOpen) setSettingsExpandedState({}); 
          if (isAdditive) {
              newState[sectionId] = !isCurrentlyOpen;
          } else {
              Object.keys(prev).forEach(key => { if (key !== sectionId && prev[key]) newState[key] = false; });
  ...
  useEffect(() => {
      const handleToggleSection = (e: CustomEvent) => {
          ...
          if (sectionId && SECTION_MODULES[sectionId]) toggleSettingsSection(sectionId, false);
      };
      const handleToggleModule = (e: CustomEvent) => {
          ...
          if (moduleId) {
              ...
              for (const [sec, modules] of Object.entries(SECTION_MODULES)) {
                  if (modules.includes(moduleId)) { parentSection = sec; break; }
              }
              if (parentSection) setSettingsOpenSections(prev => ({ ...prev, [parentSection]: true }));
          }
      };
  ...
  useEffect(() => {
    ...
    localStorage.setItem(STORAGE_KEYS.ROAD, JSON.stringify(roadConfig)); // NEW
    ...
    localStorage.setItem(STORAGE_KEYS.SHOW_ROAD, JSON.stringify(showRoad)); // NEW
    ...
    localStorage.setItem(STORAGE_KEYS.SYNC_BG_WITH_TRACK, JSON.stringify(syncBgWithTrack));
  }, [globalWaveformConfig, visualizerConfig, reactorConfig, sineWaveConfig, terrainConfig, roadConfig, dvdConfig, effectsConfig, bgColor, bgPattern, bgPatternConfig, showVisualizer, showVisualizer3D, showSineWave, showVisualizerTerrain, showRoad, showDvd, marqueeConfig, watermarkConfig, bgAutoplayInterval, cursorStyle, retroScreenCursorStyle, bgTransition, bgAnimation, isAdvancedMode, useAlbumArtAsBackground, equalizerConfig, screenFitMode, screenAlignment, youTubeConfig, syncBgWithTrack]);

  useEffect(() => { localStorage.setItem(STORAGE_KEYS.PRESETS, JSON.stringify(savedPresets)); }, [savedPresets]);
  useEffect(() => { if (activePresetId) localStorage.setItem(STORAGE_KEYS.ACTIVE_PRESET, activePresetId); else localStorage.removeItem(STORAGE_KEYS.ACTIVE_PRESET); }, [activePresetId]);
  
  // UPDATED PERSISTENCE LOGIC FOR PLAYING PLAYLIST
  useEffect(() => { 
      if (activeBgPlaylistId) localStorage.setItem(STORAGE_KEYS.ACTIVE_BG_PLAYLIST, activeBgPlaylistId); 
      // Always save playing playlist preference, even if empty (Color Mode)
      localStorage.setItem(STORAGE_KEYS.PLAYING_BG_PLAYLIST, playingBgPlaylistId); 
  }, [activeBgPlaylistId, playingBgPlaylistId]);

  // ... (hydration effect, DVD logo hydration, BG Logic functions omitted for brevity, assumed unchanged) ...
  // Full component code truncated for brevity but logic follows established patterns
  // Need to ensure loadPreset/savePreset handles roadConfig
  ...
  useEffect(() => {
    const hydrate = async () => {
      ...
      if (savedPlaying !== null) {
          // If explicitly saved as empty string, user wants color mode
          if (savedPlaying === '') {
              setPlayingBgPlaylistId('');
          } 
          // If saved as ID and exists
          else if (hydratedPlaylists.some(p => p.id === savedPlaying)) {
              setPlayingBgPlaylistId(savedPlaying);
          } 
          // Saved ID invalid/missing
          else if (hydratedPlaylists.length > 0) {
              setPlayingBgPlaylistId(hydratedPlaylists[0].id);
          }
      } else {
          // First run / No preference
          if (hydratedPlaylists.length > 0) setPlayingBgPlaylistId(hydratedPlaylists[0].id);
      }
  ...
  const updateBg = async (id: string, newFile: File) => {
      ...
      await saveBackground({ 
          id: item.id, playlistId: item.playlistId, type: item.type, file: newFile, hotspots: item.hotspots
      });
      setBgPlaylists(prev => prev.map(pl => 
          pl.id === activeBgPlaylistId ? { ...pl, items: pl.items.map(i => i.id === id ? updatedItem : i) } : pl
      ));
  };
  ...
  const updateBgMetadata = async (id: string, hotspots: BgHotspot[]) => {
      ...
      await saveBackground({ 
          id: item.id, playlistId: item.playlistId, type: item.type, file: item.file, hotspots
      });
      setBgPlaylists(prev => prev.map(pl => 
          pl.id === activeBgPlaylistId ? { ...pl, items: pl.items.map(i => i.id === id ? updatedItem : i) } : pl
      ));
  ...
  const removeBg = async (id: string) => {
      ...
      setBgPlaylists(prev => prev.map(pl => 
          pl.id === activeBgPlaylistId ? { ...pl, items: pl.items.filter(i => i.id !== id) } : pl
      ));
  };
  ...
  const selectBg = (index: number) => {
      if (activeBgPlaylistId !== playingBgPlaylistId) setPlayingBgPlaylistId(activeBgPlaylistId);
  ...
  const handleClearBg = async () => {
      ...
      if (!pl) return;
      for (const item of pl.items) await deleteBackground(item.id);
      setBgPlaylists(prev => prev.map(p => p.id === activeBgPlaylistId ? { ...p, items: [] } : p));
  };
  ...
  const savePreset = (name: string, theme?: ThemeType, controlStyle?: ControlStyle) => {
    const newPreset: AppPreset = {
      ...
      createdAt: Date.now(),
      config: {
        globalWaveformConfig, visualizerConfig, reactorConfig, sineWaveConfig, terrainConfig, roadConfig, dvdConfig,
        effectsConfig, marqueeConfig, watermarkConfig, bgColor, bgPattern, bgPatternConfig,
        showVisualizer, showVisualizer3D, showSineWave, showVisualizerTerrain, showRoad, showDvd,
        bgAutoplayInterval, syncBgWithTrack, cursorStyle, retroScreenCursorStyle, theme, controlStyle,
        bgTransition, bgAnimation, ambienceConfig: undefined, equalizerConfig, youtubeAuth: youTubeConfig
      }
    };
          ...
          if (p.id === id) {
              return {
                  ...
                  config: {
                    ...
                    effectsConfig, marqueeConfig, watermarkConfig, bgColor, bgPattern, bgPatternConfig,
                    showVisualizer, showVisualizer3D, showSineWave, showVisualizerTerrain, showRoad, showDvd,
                    bgAutoplayInterval, syncBgWithTrack, cursorStyle, retroScreenCursorStyle,
                    theme: theme || p.config.theme,
                    controlStyle: controlStyle || p.config.controlStyle,
                    bgTransition, bgAnimation, equalizerConfig, youtubeAuth: youTubeConfig
                  }
              };
          }
          return p;
  ...
  const loadPreset = (id: string): AppPreset['config'] | null => {
    ...
    setGlobalWaveformConfig(safeMerge(DEFAULT_GLOBAL_WAVEFORM_CONFIG, c.globalWaveformConfig || {}));
    ...
    setRoadConfig(safeMerge(DEFAULT_ROAD_CONFIG, c.roadConfig || {})); // NEW
    ...
    setShowRoad(c.showRoad || false); // NEW
    setShowDvd(c.showDvd);
    setBgAutoplayInterval(c.bgAutoplayInterval);
    if (c.syncBgWithTrack !== undefined) setSyncBgWithTrack(c.syncBgWithTrack);
  ...
  const exportConfig = (currentTheme?: ThemeType, currentControlStyle?: ControlStyle) => {
      const config = {
        ...
        effectsConfig, marqueeConfig, watermarkConfig, bgColor, bgPattern, bgPatternConfig,
        showVisualizer, showVisualizer3D, showSineWave, showVisualizerTerrain, showRoad, showDvd,
        bgAutoplayInterval, syncBgWithTrack, cursorStyle, retroScreenCursorStyle, bgTransition, bgAnimation,
        theme: currentTheme, controlStyle: currentControlStyle, equalizerConfig, youtubeAuth: youTubeConfig,
        version: '1.2'
      };
