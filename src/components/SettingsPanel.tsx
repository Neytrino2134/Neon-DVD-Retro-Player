...
import { Settings, Eye, Disc, Image as ImageIcon, Trash2, Activity, Layers, HardDrive } from 'lucide-react';
...
interface SettingsPanelProps {
  ...
  onClearBgMedia: () => void;
  onSelectLibraryBg: (item: CatalogItem) => void;
}
  ...
  <div className="flex items-center justify-between p-3 bg-gray-800 border border-gray-700 rounded mb-4 hover:border-gray-500 transition-colors">
    ...
    <button
      ...
      className={`relative w-12 h-6 rounded-full transition-all duration-300 shadow-inner
        ${value ? 'bg-cyan-700 shadow-[0_0_10px_#0e7490]' : 'bg-gray-700'}
      `}
    >
...
const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const handleExplorerSelect = (items: CatalogItem[]) => {
      if (items.length > 0) {
          onSelectLibraryBg(items[0]);
      }
  };

  return (
    <div className="w-full h-full flex flex-col bg-gray-900 border-r-4 border-gray-800 p-4 shadow-inner">
      <div className="mb-6 flex items-center gap-2 text-cyan-400 border-b border-gray-700 pb-2">
        <Settings className="animate-spin-slow" />
        <h2 className="text-xl font-mono shadow-neon-blue">SYSTEM</h2>
      </div>
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="flex items-center justify-center gap-2 p-2 bg-gray-800 border border-gray-600 rounded text-gray-300 hover:text-white hover:border-neon-purple hover:bg-gray-700 transition-colors"
              >
                <ImageIcon size={14} />
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 p-2 bg-gray-800 border border-gray-600 rounded text-gray-300 hover:text-white hover:border-neon-green hover:bg-gray-700 transition-colors"
              >
                <HardDrive size={14} />
